...
// Источник данных — config/users.json в Blob (при отсутствии используем ENV
// только для чтения).
//
// Поддерживает:
...
import { getAuthUsers, reloadAuthUsersFromBlob } from '../../../src/server/userStore';
import { canManageUsers, isRootAdmin } from '../../../src/domain/services/accessControl';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const currentUser = serverContext.user;
    ...
    if (!isRootAdmin(currentUser)) {
      ...
      if (roles.some((r) => forbiddenRoles.includes(r))) {
        return res
          .status(403)
          .json({ error: 'Назначать роль admin/root_admin может только администратор платформы' });
      }
    }

    const managerAllowedProfiles =
      serverContext.session?.user?.allowedProfiles?.map(String) || [];
    ...
    if (!isRootAdmin(currentUser) && profiles.length > 0 && managerAllowedProfiles.length > 0) {
      ...
      if (notAllowed.length > 0) {
        return res.status(403).json({
          error:
            'Вы не можете назначать профили, к которым у вас нет доступа',
          details: { notAllowed }
        });
      }
    }
    ...
    const existingIndex = users.findIndex((u) => String(u.id || u.username) === userId);
    ...
    if (!nextPassword) {
      ...
      return res.status(400).json({ error: 'password обязателен для нового пользователя' });
    }
    ...
    return res.status(200).json({
      user: {
        ...
        profiles: Array.isArray(updatedUser.profiles) ? updatedUser.profiles : [],
    ...
    return res
      .status(500)
      .json({ error: error?.message || 'Failed to load or update users' });
  }
}
