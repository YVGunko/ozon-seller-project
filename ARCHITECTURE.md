...
- Bootstrap и связь с файлами:
  - `profileStore.ensureProfilesLoaded()`:
    ...
    - после инициализации `config:sellers` `profileStore` используется только как fallback‑слой.
- Сохранение и использование профилей магазинов:
  - `/api/admin/sellers`:
  ...
  - `/api/profiles`:
    ...
    - фильтрация идёт по `auth.user.allowedProfiles`:
      - пустой список = доступны все Seller;
      - непустой список = только Seller с `id` из этого списка.
- UI `/admin/sellers`:
...
- Обработать смену пользователя с другими профилями (profile / warehouses):
  - при логине под пользователем, у которого нет доступа к профилю ранее залогиненного пользователя,
    сейчас возможен runtime error `"Profile is not allowed for current user"` из `useWarehouses`;
  - нужно мягко сбрасывать выбранный профиль/склад и не ломать UI (без схлопывания окна выбора профиля).
‑‑‑
